...
func _ready() -> void:
	...
	# Conectar o sinal de interação de todos os itens ao Player
	# Isso deve ser feito no _ready do World.gd ou ItemManager.gd,
	# ou de forma dinâmica se itens forem instanciados em runtime.
	# Por enquanto, vamos assumir que ItemManager fará isso.
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			# Resetar qualquer interação pendente ao iniciar um novo movimento
...
func _advance_to_next_target() -> void:
	...
	if path.is_empty():
		...
		if pending_interaction_item:
			...
			if item_manager:
				item_manager.show_item_menu(pending_interaction_item, global_position) # Passa a posição do player
...
func _find_closest_reachable_tile_world_pos(item_tile_coords: Vector2i, range: int) -> Vector2:
	var player_tile_coords = layer0.local_to_map(global_position)
	var min_dist = INF
	...
	for dx in range(-range, range + 1):
		for dy in range(-range, range + 1):
			if abs(dx) + abs(dy) <= range: # Distância de Manhattan
				var check_tile = Vector2i(item_tile_coords.x + dx, item_tile_coords.y + dy)
